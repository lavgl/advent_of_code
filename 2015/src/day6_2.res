...
module Grid = {
  ...
  let make = (size): t => Belt.Array.make(size * size, 0)
  let size = (grid: t) => {
    grid 
    -> Js.Array.length 
    -> Belt.Int.toFloat
  ...
  let totalBrightness = (grid: t) => {
    Js.Array2.reduce(grid, (a,b) => a + b, 0)
  }
}
...
let operateOnCell = (grid: Grid.t, (x,y), cmdKind) => {
  ...
  | TurnOn => set(grid[i] + 1)
  | TurnOff => set(Js.Math.max_int(grid[i] - 1, 0))
  | Toggle => set(grid[i] + 2)
  }
}
...
let main = () => {
  ...
  let res = Grid.totalBrightness(grid)
